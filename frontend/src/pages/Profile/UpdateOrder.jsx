...
export const UpdateOrder = ({orderId,IsShowUOrder,setIsShowUOrder}) => {
    ...
    const [productOrder,setproductOrder] = useState(ordersUser && ordersUser.find(item => item._id == orderId))
    const handleToggle = () =>{
  ...
  return (
     <div className='fixed inset-0 bg-[#00000075]'>
         <button onClick={() => setIsShowUOrder(!IsShowUOrder)} className='absolute top-10 z-50 right-[3%] text-white'><AiOutlineCloseCircle size={30}/></button>
          <section className='w-full max-w-[1200px] h-[100vh] overflow-y-auto mx-auto bg-gray-50 p-4 rounded-lg'>
      ...
      {productOrder && <motion.section className="Wrapper-Cart-page"
    variants={container}
    initial='hidden'
    animate='visible'
    >

      <motion.article className="flex relative flex-col py-4 px-3 gap-12 items-start md:grid md:grid-col-12"
       variants={item}
      >
        <div className="md:col-start-1 md:col-end-10 space-y-6">
          ...
          <div className="space-y-4">
          {productOrder.products.map((pro,key) =>{
        ...
        <div className="md:col-start-10 md:col-end-13 space-y-3 shadow-lg">
          ...
          <div className="flex justify-between w-full border-b border-gray-300 py-5 px-3">
           <p>Cart<span style={{opacity:'.5'}}>({productOrder.products.length})</span></p>
           <p className="font-[700] text-lg">{productOrder.totalOrder}$</p>  
          </div>
          ...
          <div className="flex justify-between w-full border-b border-gray-300 py-5 px-3">
           <p>TOTAL</p>
           <p className="font-[700] text-lg">{productOrder.Total}$</p>
          </div>

          <div>
           {/* <button onClick={handleThrowOrder} className="bg-primary-700 rounded-md py-2 text-lg front-[600] mt-3 px-2 text-white w-full block mx-auto transition duration-300 hover:bg-primary-800">throw order</button>  */}
          </div>
