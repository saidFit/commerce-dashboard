...
export const Sellers = ({successPop,setsuccessPop}) => {

  const userAuth = useSelector((state) => state.userAuth)
  const {user,Users,loading,error_single} = userAuth;
  const [Sellers,setSellers] = useState(Users && Users.filter(item => item.IsSeller))
  ...
  const handleFourFirstElement = () =>{
           setCounter(2)
    return setElementNext(Sellers.filter((item,ind)=>{if(ind <= 2 ) return item}))
  }
  ...
  const handleProveElements = () =>{
      ...
      const elements = Sellers.filter((item,ind) =>{
  ...
  const handleNextElement = () =>{
      ...
      const zeroToNext = Sellers.filter((item,index) =>{
 ...
 React.useEffect(()=>{
  if(Sellers){
      if(elementNext.length !=0)return setElementNext(Sellers.filter(item => elementNext.some(key => item._id == key._id)))
       return setElementNext(Sellers.filter((item,ind)=>{if(ind <= counter ) return item}))
 ...
 React.useEffect(()=>{
  if(Users){
    setSellers(Users.filter(item => item.IsSeller))
  }
 },[Users])
  ...
  return (
    <>
      ...
      <h1 className="relative text-xl w-fit font-[500] before:absolute before:bg-gradient-blue before:w-full before:h-0.5 before:rounded-lg before:bottom-0 before:left-0 before:right-0">Sellers</h1>
         ...
         {Users && (
          <section className='flex justify-center flex-wrap gap-8'>
            {elementNext.length !=0 && elementNext.map((item,key)=>{
              ...
              return(
                <div onClick={() => handleShowStateUser(_id)} className='bg-gray-50 space-y-12 rounded-md border border-gray-200 w-[25%]'>
