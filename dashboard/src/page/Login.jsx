 ...
 const handleSubmit = (e) =>{
   ...
   const formData = new FormData()
    ...
    return (
         ...
         <section className='bg-[#0000007a] z-50 b'>
               <div style={{...absoluteCenter}} className='container text-sm mx-auto space-y-8 rounded-md  p-3 shadow-My-box bg-gray-0 border border-gray-300 w-full max-w-[800px] dark:bg-slate-800 dark:border'>
               <h1 className="relative text-xl w-fit font-[500] md:ml-6 before:absolute before:bg-gradient-blue before:w-full before:h-0.5 before:rounded-lg before:bottom-0 before:left-0 before:right-0">Log in to Dashboard</h1>

            <form className='space-y-6' onSubmit={handleSubmit}>
                    <div className='flex items-center'>
                        <label className='w-[20%] text-lg'>email*</label>
                        <input value={email} onChange={(e) =>setemail(e.target.value)} className={error_arr.includes('email')?'border w-[80%] py-2 px-4 outline-none border-danger-600 rounded-md shadow-My-box dark:bg-slate-800 dark:border dark:border-black':'border w-[80%] py-2 px-4 outline-none border-gray-300 rounded-md shadow-My-box dark:bg-slate-800 dark:border dark:border-black'} type='email' placeholder='Enter-email' name='email' />
                    </div>
                    <div className='relative flex items-center'>
                        <label className='w-[20%] text-lg'>password*</label>
                        <input onChange={(e)=>setpassword(e.target.value)} value={password}className={error_arr.includes('password')?'border w-[80%] py-2 px-4 outline-none border-danger-600 rounded-md shadow-My-box dark:bg-slate-800 dark:border dark:border-black':'border w-[80%] py-2 px-4 outline-none border-gray-300 rounded-md shadow-My-box dark:bg-slate-800 dark:border dark:border-black'} type={IsShowPss?'text':'password'} name='password' placeholder='password' />
